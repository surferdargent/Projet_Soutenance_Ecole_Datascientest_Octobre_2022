...
def mean_rolling(df, x, y):
    """Les moyennes roulantes."""
    new_df = df
# Moyenne roulante des victoires par joueur sur x et y mois en fonction
# des victoires totales, de la surface, du tournois et des tours
...
new_df_preprocessing = new_df_preprocessing.drop(['Player', 'Location', 'Tournament', 'Series', 'Round', 'Name', 'Tourna', 'Type_series', 'Type_round'], axis=1)

# Passer les variables object numériques en float

new_df_preprocessing[['BestOf', 'Court', 'Surface', 'Rank', 'SetsWon', 'EloPoints', 'B365', 'RankDiff', 'Win', 'Id_player', 'Id_tournament', 'Id_series', 'Id_round']]=new_df_preprocessing[['BestOf', 'Court', 'Surface', 'Rank', 'SetsWon', 'EloPoints',
...
def split(data):
    """Séparation des datasets et normalisation."""
    df = pd.DataFrame()
...
st.markdown("""
Après avoir normalisé et entrainé les modèles, nous avons obtenu des scores
 qui sont au dessus de 95 %.
Les performances de nos modèles laissent penser qu’il y a sans doute.
un surapprentissage, qu’il nous faut identifier.
...
st.markdown("""
Nous constatons que la variable "SetsWon" est fortement corrélée avec "Win"
ce qui est normal car elle donne une indication sur le nombre de sets gagnés
sur le match.
Par contre cette information nous ne l'avons pas avant le match il faut
donc la supprimer des features et relancer le modèle.
...
def importance_variables():
    """Répartition des importances des variables."""
...
graph_imp_va = importance_variables()
st.markdown("""
...
drop_variable1 = ['EloPoints', 'RankDiff', 'Ratio_tours_6_mois',
                  'Ratio_victoire_6_mois', 'Ratio_surface_6_mois',
                  'Ratio_tournois_6_mois', 'Ratio_tournois_18_mois',
                  'Ratio_tours_18_mois', 'Ratio_victoire_18_mois',
                  'Ratio_surface_18_mois']
...
drop_variable2 = ['RankDiff',
                  'Ratio_tours_6_mois', 'Ratio_victoire_6_mois',
                  'Ratio_surface_6_mois', 'Ratio_tournois_6_mois',
                  'Ratio_tournois_18_mois', 'Ratio_tours_18_mois',
                  'Ratio_victoire_18_mois', 'Ratio_surface_18_mois']
...
drop_variable3 = ['Ratio_tours_6_mois', 'Ratio_victoire_6_mois',
                  'Ratio_surface_6_mois', 'Ratio_tournois_6_mois',
                  'Ratio_tournois_18_mois', 'Ratio_tours_18_mois',
                  'Ratio_victoire_18_mois', 'Ratio_surface_18_mois']
...
# "Features d'origine + Points ELO + Diff. de classement + Moy.roul 6 mois"
#  à conserver
drop_variable4 = ['Ratio_tournois_18_mois', 'Ratio_tours_18_mois',
                  'Ratio_victoire_18_mois', 'Ratio_surface_18_mois']
df4 = new_df_preprocessing_demo.drop(drop_variable4, axis=1)
df4 = split_normalisation(df4, 4)


# "Features d'origine + Points ELO + Diff. de classement + Moy.roulantes 6 mois
#  + Moy.roulantes 18 mois"  à conserver
df5 = new_df_preprocessing_demo
df5 = split_normalisation(df5, 5)


st.markdown(""" Bilan des performances des différentes options.""")
...
def creat_df():
    """Création de df pour voir le score de chaque features."""
...
selections = ["Features d'origine", "Features d'origine + Pts ELO",
              "Features d'origine + Pts ELO + Diff. de classement",
              "Features d'origine + Pts ELO + Diff. de classement"
              + "Moy.roulantes 6 mois", "Features d'origine" + "Pts ELO"
              + "Diff. de classement + Moy.roulantes 6 mois" +
              "Moy.roulantes 18 mois"]
...
if selection == selections[0]:
    ...
    st.markdown("""
                Nous observons que le Random Forest obtient les meilleurs
                résultats mais nous pouvons encore les améliorer en optimisant
                les hyperparamètres.
                      """)
st.markdown("""
        - L'optimisation
...
st.markdown(
    """ Nous avons testé trois stratégies différentes et évaluer laquelle
    permet de maximiser le profit. Pour ces trois stratégies notre mise
    de départ est 10 euros par défaut, et nous établissons les gains obtenus
    sur l’ensemble des matchs qui se sont déroulés entre 01 janvier 2016
    au 04 mars 2018, soit notre échantillon de test.
  """
  )
...
def paris2(gain=0, mise_totale=0, mise_de_depart=mise_de_depart, seuil=0.8):
    """Gains en suivant le mode et coeff."""
    y_pred_proba = probs
...
st.markdown("""
    La somme pariée serait de 20600 euros et le gain prédit de 7843
    euros.Soit 38.0 % de bénéfices""")

st.markdown("""
    La dernière stratégie serait la plus optimale car le bénéfice
    est de 38,0 % légèrement supérieur au bénéfice des préconisations
    bookmakers
    ( 36,0 %)  Surtout la somme engagée pour la dernière stratégie est de 20600
    euros contre 40970 euros si on suit les préconisations bookmakers.
    Notre stratégie battrait les bookmakers...
            """)
